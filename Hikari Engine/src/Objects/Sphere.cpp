	...
	*/
	...
	for(; r < rings + 1; r++)
	{
		for(s = 0; s < segments; s++)
		{
			...
			m_pVertices[cur_pos].position_modelSpace.x( radius * cos(s_f * s) * cos(-M_PI_2 + r_f * r) + center.x() );
			m_pVertices[cur_pos].position_modelSpace.y( radius * sin(-M_PI_2 + r_f * r) + center.y() );
			m_pVertices[cur_pos].position_modelSpace.z( radius * sin(s_f * s) * cos(-M_PI_2 + r_f * r) + center.z() );
			
			// UV wierzcho³ków
			d = Vector3D(m_pVertices[cur_pos].position_modelSpace, center);
			m_pVertices[cur_pos].uv.x(0.5 + atan2(d.x(), d.y()) / M_PI_2);
			m_pVertices[cur_pos].uv.y(0.5 + asin(d.y()) / M_PI);

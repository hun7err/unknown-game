#include "../../../include/Core/Materials/Material.hpp"
...
void Hikari::Material::initialize(void)
{
	...
	m_hDiffuseMap = nullptr;
	m_hBumpMap = nullptr;
	m_hSpecularMap = nullptr;
	m_hNormalMap = nullptr;
	m_Shininess = 0.5f;
}
...
{
	return m_hDiffuseMap;
}

void Hikari::Material::diffuseMap(Hikari::HTexture hNewDiffuseMap)
{
	if(hNewDiffuseMap.isValid())
	{
		m_hDiffuseMap = hNewDiffuseMap;
	}
}

Hikari::HTexture Hikari::Material::specularMap(void)
{
	return m_hSpecularMap;
}

void Hikari::Material::specularMap(Hikari::HTexture hNewSpecularMap)
{
	if(hNewSpecularMap.isValid())
	{
		m_hSpecularMap = hNewSpecularMap;
	}
}
...
void Hikari::Material::normalMap(Hikari::HTexture hNewNormalMap)
{
	if(hNewNormalMap.isValid())
	{
		m_hNormalMap = hNewNormalMap;
	}
}
...
void Hikari::Material::bumpMap(Hikari::HTexture hNewBumpMap)
{
	if(hNewBumpMap.isValid())
	{
		m_hBumpMap = hNewBumpMap;
	}
}

Hikari::HShader Hikari::Material::shader(void)
{
	return m_hShaderProgram;
}

void Hikari::Material::shader(Hikari::HShader hNewShader)
{
	if(hNewShader.isValid())
	{
		m_hShaderProgram = hNewShader;
	}
}

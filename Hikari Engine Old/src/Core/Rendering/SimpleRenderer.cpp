...
// to-do:
/*
	+ czym w³aœciwie jest world matrix? tym co macierz modelu? (rastertek: GetWorldMatrix() z m_D3D)
	A: tak, to jest macierz modelu

	- przekazywanie world, view i projection -> gdzie? (renderer? draw() obiektu?) + zrobiæ to
	- assimp
	- rendering kul/pude³/szeœcianów itp. + nowe obiekty? (cone chocia¿by, torus?)
	- bullet
	- directsound?
	- obiekty 2D (ko³o, prostok¹t z zaokr¹glonymi rogami, kwadrat, prostok¹t, elipsa itp.), GUI
	- LUA - framework + api
	- i18n/l18n, s³owniki dla ró¿nych jêzyków (Translator: mapa <char[2], Dictionary> {zamiast char[2] mo¿e byæ std::string} a Dictionary to map<std::string, std::wstring> )
*/

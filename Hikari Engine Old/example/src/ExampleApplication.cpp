...
void ExampleApplication::yawCameraPlus(void)
{
...
void ExampleApplication::yawCameraMinus(void)
{
	float yaw = Hikari::Engine::camera()->yaw();
	yaw -= 0.001f;
	Hikari::Engine::camera()->yaw(yaw);
...
void ExampleApplication::run(void)
{
	...
	*/
	...
	std::function<void(void)> yawCameraPlusFun = std::bind(&ExampleApplication::yawCameraPlus, this);
	Hikari::Engine::input()->keyHandler(VK_RIGHT, yawCameraPlusFun);
	std::function<void(void)> yawCameraMinusFun = std::bind(&ExampleApplication::yawCameraMinus, this);
	Hikari::Engine::input()->keyHandler(VK_LEFT, yawCameraMinusFun);
	...
	Hikari::Object *pTriangle = new Hikari::Objects::Triangle(
		Hikari::Vector3D(0.0f, 0.5f, 1.0f),
		Hikari::Vector3D(0.55f, -0.5f, 1.0f),
		Hikari::Vector3D(-0.55f, -0.5f, 1.0f)
	);
